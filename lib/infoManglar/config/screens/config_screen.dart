...
class _State extends State<ConfigScreen> {
  ...
  final Option versionOption = Option(
      label: 'Versión',
      type: 'input',
      typeInput: 'textPersonName',
      placeholder: 'Ejemplo: 1.0.7');
  ...
  Widget build(BuildContext context) {
    ...
    final orgTable = TableList(
      ...
      onRemove: (int organizationManglarId) =>
          _onRemoveOrg(organizationManglarId),
    ...
    return Container(
      child: ListView(
        children: <Widget>[
          ...
          _loaded
              ? _getTableListAllowedUsers(
                  'super-admin', 'Super Administradores')
              : Utility.getCircularProgress(),
          _loaded
              ? _getTableListAllowedUsers('org', 'ORG Administradores')
              : Utility.getCircularProgress(),
          _loaded
              ? _getTableListAllowedUsers('mae', 'MAE Administradores')
              : Utility.getCircularProgress(),
          _loaded
              ? _getTableListAllowedUsers('inp', 'IPIAP Administradores')
              : Utility.getCircularProgress(),
          _loaded
              ? _getTableListAllowedUsers('socio', 'Socios')
              : Utility.getCircularProgress(),
        ],
      ),
    );
  ...
  Future<Null> _loadData() async {
    ...
    _superAdminsAllowed =
        await _configRepository.getAllowedUsers('super-admin');
  ...
  TableList _getTableListAllowedUsers(String userType, String title) {
    ...
    if (userType == 'super-admin')
      data = _getDataAllowedUsers(_superAdminsAllowed);
  ...
  _onRemoveOrg(int organizationManglarId) {
    Utility.showConfirm(context, 'Atención',
        '¿Esta seguro de remover al usuario id: $organizationManglarId?', () {
      Utility.showLoading(context);
      _configRepository
          .removeOrganizationManglar(organizationManglarId)
          .then((Data result) {
  ...
  _onRemove(int allowedUserId) {
    Utility.showConfirm(context, 'Atención',
        '¿Esta seguro de remover al usuario id: $allowedUserId?', () {
      Utility.showLoading(context);
  ...
  _changeVersion(context) {
    if (_configForm == null) {
      Utility.showToast(
          context, 'No se logró obtener la configuración de versión actual');
      return;
    }
    if (!versionOption.hasValue()) {
      Utility.showToast(
          context, 'Debe llenar el valor de version ejemplo 1.0.7');
      return;
    }
    ...
    Utility.showConfirm(context, 'Atención',
        '¿Esta seguro de cambiar la versión de ${_configForm.version} a $newVersion?',
        () {
      Utility.showLoading(context);
